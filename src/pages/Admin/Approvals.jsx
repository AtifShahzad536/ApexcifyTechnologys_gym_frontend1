...
const Approvals = () => {
    ...
    const handleApprove = async () => {
        ...
        try {
            const userInfo = JSON.parse(localStorage.getItem('userInfo'));
            await axios.put(`${import.meta.env.VITE_API_URL}/approvals/${selectedUser._id}/approve`, {
                role,
    ...
    const handleReject = async (userId) => {
        ...
        try {
            const userInfo = JSON.parse(localStorage.getItem('userInfo'));
            await axios.put(`${import.meta.env.VITE_API_URL}/approvals/${userId}/reject`, {}, {
                headers: {
                    Authorization: `Bearer ${userInfo.token}`
                }
            });
            ...
            <AnimatePresence>
                {showModal && selectedUser && (
                    ...
                    >
                        ...
                        >
                            <h2 className="text-2xl font-bold text-gray-800 mb-4">Approve User</h2>
                            <p className="text-gray-600 mb-6">
                                Approving: <span className="font-semibold">{selectedUser.name}</span>
                            </p>
                                        ...
                                        >
                                            {students.map(student => (
                                                <option key={student._id} value={student._id}>
                                                    {student.name} - {student.studentClass?.name || 'No Class'} ({student.rollNumber || 'No Roll'})
                                                </option>
                                            ))}
                                        </select>
